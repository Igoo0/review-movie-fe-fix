...
const MovieDetail = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        // Fetch movie using consistent API endpoint
  ...
  const handleDeleteReview = async (reviewId) => {
    ...
    if (window.confirm('Are you sure you want to delete this review?')) {
      try {
        console.log('Sending DELETE request to:', `https://buku-tukar-559917148272.us-central1.run.app/reviews/${reviewId}`);
        
        const response = await axios.delete(`https://buku-tukar-559917148272.us-central1.run.app/reviews/${reviewId}`);
        ...
        <div className="column is-4">
          <div className="poster-container">
            <figure className="image">
              <img 
                src={movie.poster ? movie.poster : 'https://via.placeholder.com/400x600/333/fff?text=No+Poster'} 
                alt={movie.name}
